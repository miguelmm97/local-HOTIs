"""
This code runs the simulations discussed in the main text. The code produces an output data .h5 file with the data of
the corresponding simulation. The code can be directly run provided the appropriated packages are in the virtual
environment (provided in environment.yml).

The code is written to produce a simulation from scratch. In order to reproduce the data for the figures presented
in the main text, one can use this code together with data from the different simulations given in the data folder.
We specify how to do this in a commented out section.
...
from Hamiltonian_Kwant import spectrum, local_DoS, Hamiltonian_Kwant, OPDM, reduced_OPDM
from colorbar_marker import get_continuous_cmap
...
#%% Variables
gamma             = 0.5                          # float: gamma parameter in Eq.(12) in the main text
eta               = 1                             # float: eta parameter in Eq.(12) in the main text
width             = 0.1                          # float: width of the amorphous distribution
r                 = 1.3                          # float: cutoff distance r_0 in Eq.(17) in the main text
Nx                = 20                           # int: number of sites along x
Ny                = 20                           # int: number of sites along y
Nsites            = Nx * Ny
cutx, cuty        = 0.5 * Nx, 0.5 * Ny           # Cuts defining the A region
center_theta      = 0.5 * cutx                   # Position of the shell (c in Eq.(13) in the main text)
sharpness_theta   = 0.5 * cutx                   # Sharpness of the transition (\omega in Eq.(13) in the main text)
params_dict = {'gamma': gamma, 'eta': eta}
crystalline = False                              # if True creates a crystalline lattice without taking width into account
C4symmetry = True                                # if True imposes C4symmetry in the lattice
...
# Theta function selecting a single corner
...
# Lattice
loger_main.info('Generating crystalline/amorphous structure...')
...
# To reproduce the simulations in the main text
# loger_main.info('Generating crystalline/amorphous structure...')
# file_list = ['fig1-data.h5']
